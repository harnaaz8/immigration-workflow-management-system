...
function EnrollmentDashboard() {
  ...
  const officerName = localStorage.getItem("name") || "Enrollment Officer";
  ...
  const loadStats = async () => {
    try {
      setLoading(true);
      const res = await api.get(`/students/assigned/enrollment/${officerName}`);
      
      if (res.data && Array.isArray(res.data)) {
        setStudentsCount(res.data.length);

        setCompletedCount(
          res.data.filter((s) => s.status === "VISA" || s.status === "COMPLETED").length
        );

        setPendingCount(
          res.data.filter((s) => s.status === "ENROLLMENT").length
        );
      }
    } catch (err) {
      console.error("Failed to load enrollment workstation metrics:", err);
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div style={{ display: "flex", justifyContent: "center", alignItems: "center", minHeight: "100vh", flex: 1, background: "#f8fafc" }}>
        ...
        <div style={{ textAlign: "center" }}>
          ...
          <h3 style={{ color: "#475569", margin: 0, fontSize: "16px" }}>Aggregating Enrollment Ledgers...</h3>
        </div>
      </div>
    );
  }
    ...
    <div style={{ flex: 1, maxWidth: "1200px", margin: "0 auto", padding: "40px 30px", boxSizing: "border-box", background: "#f8fafc", minHeight: "100vh" }}>
      ...
      <div style={{ marginBottom: "35px" }}>
        ...
        <p style={{ margin: "6px 0 0 0", color: "#64748b", fontSize: "15px" }}>
          Logged in: <strong style={{ color: "#334155" }}>{officerName}</strong> — Direct institutional onboarding, COE confirmations, and pipeline transitions.
        </p>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: "24px", marginBottom: "40px" }}>
        ...
        <div style={{ borderLeft: "4px solid #22c55e", borderRadius: "4px" }}>
          <StatsCard title="Completed & Promoted" value={completedCount} />
        </div>
        <div style={{ borderLeft: "4px solid #f59e0b", borderRadius: "4px" }}>
          <StatsCard title="Pending Institutional Stage" value={pendingCount} />
        </div>
      </div>
      ...
      <div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(320px, 1fr))", gap: "20px" }}>
          ...
          >
            ...
            <div>
              <strong style={{ display: "block", color: "#0f172a" }}>Process Enrollment Queues</strong>
              <span style={{ fontSize: "13px", color: "#64748b", fontWeight: "normal" }}>Review confirmation letters, upload invoices, and allocate IDs.</span>
            </div>
          </button>
          ...
          >
            ...
            <div>
              ...
              <span style={{ fontSize: "13px", color: "#64748b", fontWeight: "normal" }}>Manage security variables and terminal tracking configurations.</span>
            </div>
          </button>
